...
FROM quay.io/gt4sd/gt4sd-base:v1.1.8-cpu
ENV PATH=/opt/conda/envs/gt4sd/bin/:${PATH}
COPY notebooks/requirements.txt notebooks_requirements.txt
RUN pip install --no-cache-dir notebook==5.* tensorflow-cpu>=2.1.0 gt4sd>=1.1.8 && pip install --no-cache-dir -r notebooks_requirements.txt
