...
FROM drugilsberg/gt4sd-base:main-20221013085640
ENV PATH=/opt/conda/envs/gt4sd/bin/:${PATH}
COPY notebooks/requirements.txt notebooks_requirements.txt
RUN pip install --no-cache-dir notebook==5.* tensorflow-cpu>=2.1.0 gt4sd>=0.54.0 && pip install --no-cache-dir -r notebooks_requirements.txt
