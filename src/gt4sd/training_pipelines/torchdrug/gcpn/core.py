    ...
    def train(  # type: ignore
        ...
        """Generic training function for training a Graph Convolutional Policy Network
            (GCPN) model. For details see:
                You, J. et al. (2018). Graph convolutional policy network for goal-
                directed molecular graph generation.
        ...
        try:
            ...
            model = RGCN(
                input_dim=params.get("input_dim", dataset.node_feature_dim),
                num_relation=params.get("num_relation", dataset.num_bond_type),
            ...
            task = GCPNGeneration(
                ...
                criterion=ast.literal_eval(params["criterion"]),
                hidden_dim_mlp=params.get("hidden_dim_mlp", 128),
            ...
            logger.exception(
                "Exception occurred while running TorchDrugGCPNTrainingPipeline"
            )


@dataclass
class TorchDrugGCPNModelArguments(TrainingPipelineArguments):
    ...
    hidden_dim_mlp: int = field(
        default=128,
        metadata={"help": "Hidden size of GCPN internal MLP."},
    )
